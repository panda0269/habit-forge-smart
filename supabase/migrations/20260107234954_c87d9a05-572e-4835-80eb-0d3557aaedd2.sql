-- Fix the get_leaderboard function to calculate completion rate correctly
-- Use actual days since first log instead of fixed 30 days
...
BEGIN
  ...
  SELECT 
    ...
    -- Calculate completion rate: completions / (habits * days_tracked), capped at 100
    CASE 
      WHEN COUNT(DISTINCT h.id) > 0 THEN 
        LEAST(100, ROUND(
          (COUNT(DISTINCT hl.id)::numeric / 
          GREATEST(1, COUNT(DISTINCT h.id) * 
            GREATEST(1, (CURRENT_DATE - MIN(h.created_at::date) + 1))
          )) * 100, 1
        ))
      ELSE 0 
